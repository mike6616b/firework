...
function getParticleSprite(color) {
    if (!particleSprites[color]) {
        const sCanvas = document.createElement('canvas');
        sCanvas.width = 20;
        sCanvas.height = 20;
        const sctx = sCanvas.getContext('2d');
        const g = sctx.createRadialGradient(10, 10, 0, 10, 10, 10);
        ...
        sctx.fill();
        particleSprites[color] = sCanvas;
    }
    return particleSprites[color];
}
...
class Particle {
    ...
    reset(x, y) {
        ...
        this.sprite = getParticleSprite(this.color);
    ...
    update(delta) {
        ...
        // 粒子逐漸減速
        this.speedX *= Math.pow(this.decay, dt);
        this.speedY *= Math.pow(this.decay, dt);
...
function getParticle(x, y) {
    return (particlePool.pop() || new Particle(x, y)).reset(x, y);
}
...
function animate(current) {
    const delta = (current - lastTime) / BASE_FRAME_TIME;
    lastTime = current;
    // 檢查粒子數量，如果太多可能導致效能問題，則移除一些
    if (particles.length > 200) {
        const excess = particles.splice(0, particles.length - 200);
        excess.forEach(releaseParticle);
    }
    ...
    // 更新指針位置（緩和移動）
    pointer.x += (mouseX - pointer.x) * 0.2 * delta;
    pointer.y += (mouseY - pointer.y) * 0.2 * delta;
